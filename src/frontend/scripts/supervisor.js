document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderizarTarefas(tarefas) {
    ...
    tarefas.forEach((tarefa) => {
      ...
      li.innerHTML = `
                ...
                <span class="badge ${
                  tarefa.estaConcluida ? "bg-success" : "bg-warning text-dark"
                }">
  ...
  if (btnVerPendentes && selectFuncionario) {
    btnVerPendentes.addEventListener("click", async () => {
      ...
      try {
        const resposta = await fetch(
          `http://localhost:3000/supervisor/pendentes/${idFuncionario}`
        );
        ...
        Swal.fire({
          title: "Erro!",
          text: "Erro ao buscar tarefas pendentes.",
          icon: "error",
          confirmButtonText: "OK",
        });
      }
  ...
  if (btnVerConcluidas && selectFuncionario) {
    btnVerConcluidas.addEventListener("click", async () => {
      ...
      try {
        const resposta = await fetch(
          `http://localhost:3000/supervisor/concluidas/${idFuncionario}`
        );
        const tarefas = await resposta.json();
        renderizarTarefas(tarefas);
      } catch (error) {
        Swal.fire({
          title: "Erro!",
          text: "Erro ao buscar tarefas concluídas.",
          icon: "error",
          confirmButtonText: "OK",
        });
      }
  ...
  if (btnVerTodas && selectFuncionario) {
    btnVerTodas.addEventListener("click", async () => {
      ...
      try {
        const resposta = await fetch(
          `http://localhost:3000/supervisor/todas/${idFuncionario}`
        );
        const tarefas = await resposta.json();
        renderizarTarefas(tarefas);
      } catch (error) {
        Swal.fire({
          title: "Erro!",
          text: "Erro ao buscar todas as tarefas.",
          icon: "error",
          confirmButtonText: "OK",
        });
      }
    ...
    formCriar.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (resposta.ok) {
          ...
          mensagem.textContent = `Tarefa criada com sucesso! ID: ${
            data.tarefaId || data.id
          }`;
      ...
      try {
        ...
        if (resposta.ok) {
          mensagem.textContent =
            data.mensagem || "Tarefa associada com sucesso!";
          formAssociar.reset();
          if(data.erro || data.error) mensagem.textContent = data.erro || data.error || "Erro ao associar tarefa.";
        } else {
          mensagem.textContent = data.erro || data.error || "Erro ao associar tarefa.";
        }
      } catch {
